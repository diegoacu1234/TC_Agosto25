{-# OPTIONS_GHC -fno-warn-tabs #-} --pragma para que no llore si usamos tabs
...
} -- esta fue con case, como toda la vida

eval m (Conj e1 e2) = let (m',b1) = eval m e1  -- ver que "asigno" variables y luego las puedo usar
					  in let(m'',b2) = eval m' e2
						 in (m'', b1 && b2)
			
eval m (Neg e) = (m', not b) 
				where (m',b) = eval m e --parecido al let solo que aclaro al final
				
eval m (Var x) = let Just b = lkup x m
					in (m,b)
			
eval m (Asig x e) = let (m',v) = eval m e
					in (upd x v m,v)
